...
const InputSanitizer = function () {
  ...
  self.sanitize = function (rawBlip) {
    ...
    blip.changeType = sanitizeHtml(blip.changeType, restrictedOptions)
  ...
  self.sanitizeForProtectedSheet = function (rawBlip, header) {
    ...
    const changeTypeIndex = header.indexOf('changeType')
    ...
    const changeType = changeTypeIndex === -1 ? '' : blip[changeTypeIndex]
    ...
    blip.changeType = sanitizeHtml(changeType, restrictedOptions)
